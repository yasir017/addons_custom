  ...
  'use strict';
    ...
    get disable_refund() {
      if (this.env.pos.config.module_pos_hr) {
       const cashierId = this.env.pos.get_cashier().id;
       const sessionAccess = this.env.pos.session_access.find(access => access.id === cashierId);
       if (sessionAccess && sessionAccess.disable_refund) {
           self.$('#customer_refund').css({ "border": "1px solid #C9CCD2", "background-color": "#E0E2E6","color":"#666666" });
       }
       return sessionAccess ? sessionAccess.disable_refund : false;
   }
   else {return false;}
   }
